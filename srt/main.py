...
class Subtitle:
    def __init__(self, source: str):
        ...
        for index, line in enumerate(self._lines):
            if not line:
                self.displayed_lines.append(DisplayedLine.instance(self._lines[start_index:index]))
                start_index = index + 1

    def sync_by_seconds(self, seconds: float) -> Subtitle:
        """
...
if __name__ == '__main__':
    ...
    if args.sync != 0:
        Subtitle(args.input).sync_by_seconds(args.sync).to_file(args.output)
